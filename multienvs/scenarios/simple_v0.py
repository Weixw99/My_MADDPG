...
# 原论文中将对手设置为agent，在此主要体现多USV的协作，因此将landmark设置对手的性质
...
class Scenario(BaseScenario):
    def make_world(self):
        ...
        world = World()
        world.collaborative = True  # 设置世界是协作的
        # set any world properties first
        world.dim_c = 2  # 设置维度
        num_static_landmarks = 0  # landmark直译是地标性建筑
        num_dynamic_landmarks = 2  # 此处设置两个动态的landmark，分别为动态的目标点和动态障碍物
        num_agents = 3  # agent的数量
        num_landmarks = num_static_landmarks + num_dynamic_landmarks
        # add agents
        world.agents = [Agent() for i in range(num_agents)]  # 根据agent的数量创建list
        for i, agent in enumerate(world.agents):  # enumerate对数据对象进行遍历，返回索引和data
        ...
        for i, landmark in enumerate(world.landmarks):
            ...
            # 先对动态landmark进行设置，一个设置障碍物为对手，另一个为目标点，不是对手
            landmark.adversary = True if i < num_dynamic_landmarks else False
    ...
    def reset_world(self, world):
        ...
        for i, agent in enumerate(world.agents):
            ...
            agent.state.p_pos = np.array([np.random.uniform(-1, 1), np.random.uniform(-0.7, 0)])  # 设置agent的初始位置
            agent.state.p_vel = np.zeros(world.dim_p)  # 设置agent初始速度
            agent.state.c = np.zeros(world.dim_c)  # 应该是维度

        # random properties for landmarks
        for i, landmark in enumerate(world.landmarks):
            if i == 0:
                landmark.color = np.array([0.25, 0.25, 0.25])  # 设置障碍物的颜色
                landmark.state.p_pos = np.array([-0.97, 0.5])  # 设置障碍物的位置
            else:
                landmark.color = np.array([0.85, 0.35, 0.35])  # 设置目标点的颜色
                landmark.state.p_pos = np.array([0.66, 1.3])  # 设置目标点的位置
            landmark.state.p_vel = np.zeros(world.dim_p)  # 设置障碍物和目标点的速度
    ...
    def is_collision(self, agent1, agent2):  # 判断两者是否碰撞的函数，输出布尔值
    ...
    def observation(self, agent, world):
        ...
        return np.concatenate(
            [agent.state.p_vel] + [agent.state.p_pos] + entity_pos + entity_vel + other_pos + other_vel)
